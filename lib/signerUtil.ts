...
export const getSigner = async (
  ...
  const signer: EvmSigner = await initSigner(address, extension.signer);

  let addr = await signer.getAddress();
  console.log("Signer address=", addr, await signer.getSubstrateAddress());
    ...
    if (balance.lt("3")) {
      throw new Error(
        "<p>To enable contract interaction you need to sign transaction with ~3REEF fee.<br/>To get 1000 testnet REEF simply type:<br/> <code>!drip " +
          address +
          '</code> <br/>in <a href="https://app.element.io/#/room/#reef:matrix.org" target="_blank">Reef matrix chat</a>. Refresh this page after receiving testnet REEF (~10sec).</p>'
      );
    }
  }
...
export async function getMnemonicSigner_serverSideOnly(): Promise<EvmSigner> {
  const sigKey = new MnemonicSigner(
    "promote common nuclear marriage steak gain elder grief will domain long piano"
  );
  return new EvmSigner(
    await initProvider(),
    "5Cz2J8Fi2qPHYRgu9yWLqCTLC6RJLLik8pq77n1DjPNLgies",
    sigKey
  );
}
...
class MnemonicSigner implements SignerInterface {
  ...
  async signPayload(payload: SignerPayloadJSON): Promise<SignerResult> {
    ...
    const pair: KeyringPair =  keyring.addFromMnemonic(this.mnemonic);
  ...
  async signRaw(payload: SignerPayloadRaw): Promise<SignerResult> {
    ...
    const { wrapBytes } = await import("@reef-defi/extension-dapp");
